...
# create-autorun.sh
#
# This version was coded on 2017/02/05 by OneCD and you can blame him if it all goes horribly wrong. ;)
#
...
Init()
	...
	local SCRIPT_VERSION="2017.02.05.01"
...
BackupExistingAutorun()
	...
	if [ -e "$AUTORUN_LINK_PATHFILE" ]; then
		...
		if [ -e "$backup_pathfile" ] ; then
			mv "$backup_pathfile" "$backup_pathfile.1"
