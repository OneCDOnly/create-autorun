...
# create-autorun.sh
#
# This version was coded on 2017/02/04 by OneCD and you can blame him if it all goes horribly wrong. ;)
#
...
Init()
	...
	local SCRIPT_VERSION="2017-02-04.02"
...
ConfirmDOMPartition()
	...
	if [ "$partition_size" != "UNKNOWN" ]; then
		if [ "$partition_size" -eq "$DOM_PARTITION_SIZE_EXPECTED" ]; then
		...
		ShowFailed "Unable to confirm DOM partion size"
	fi
...
BackupExistingAutorun()
	...
	if [ -e "$AUTORUN_LINK_PATHFILE" ]; then
		...
		if [ -e "$backup_pathfile" ] ; then
			adder=1
			while [ -e "$backup_pathfile.$adder" ]; do
				((adder++))
				[ "$adder" -gt "1000" ] && break
			done
			backup_pathfile="$backup_pathfile.$adder"
		fi
...
[ "$exitcode" -eq "0" ] && CreateDOM_mount_point
